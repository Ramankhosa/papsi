...
export default function RelatedArtStage({ session, patent, onComplete, onRefresh }: RelatedArtStageProps) {
  ...
  useEffect(() => {
    console.log('=ƒÆí ideaBank state changed:', ideaBank.length, 'ideas')
    if (ideaBank.length > 0) {
      console.log('=ƒÆí First idea title:', ideaBank[0]?.title)
    }
  }, [ideaBank])
  ...
  useEffect(() => {
    console.log('=ƒöä useEffect running - session changed, ideaBank.length:', ideaBank.length, 'customQuery:', customQuery, 'results.length:', results.length)
  ...
  const runSearch = async () => {
    console.log('=ƒÜÇ runSearch called - customQuery:', customQuery, 'q:', q)
    try {
      setBusy(true)
      setSearching(true)
      setError(null)
      ...
      // Debug logging
      console.log('=ƒöì Search Query Debug:')
      ...
      const progressSteps = [
        '=ƒöì Scanning through 12M+ global patent database...',
        '=ƒÄ» Applying advanced semantic analysis to your invention...',
        '=ƒºá Using proprietary AI algorithms for relevance matching...',
        '=ƒôè Calculating multi-dimensional similarity scores...',
        '=ƒö¼ Cross-referencing with CPC/IPC classification systems...',
        'GÜí Filtering results through novelty assessment engine...',
        'G£¿ Ranking patents by technical relevance and impact...',
        '=ƒôï Preparing final results with comprehensive metadata...'
      ]
      ...
      setSearchProgress('=ƒÆí Finalizing patent analysis and generating comprehensive report...')
      ...
      setSearchProgress(`G£¿ Analysis complete! Found ${items.length} highly relevant patent${items.length !== 1 ? 's' : ''} from millions of global records.`)
  ...
  const runAIReview = async () => {
    ...
    try {
      setError(null)
      setReviewing(true)
      setReviewInfo('Analyzing patents with AIGÇª')
      ...
      if (Array.isArray(resp?.ideaBankSuggestions)) {
        ...
        console.log('G£à Found ideas in resp.ideaBankSuggestions')
      } else if (Array.isArray(resp?.data?.ideaBankSuggestions)) {
        ideas = resp.data.ideaBankSuggestions
        console.log('G£à Found ideas in resp.data.ideaBankSuggestions')
      } else if (resp?.data && Array.isArray(resp.data.ideaBankSuggestions)) {
        ideas = resp.data.ideaBankSuggestions
        console.log('G£à Found ideas in resp.data (nested)')
      } else {
        console.log('G¥î No ideas found in any expected location')
      }
      ...
      console.log('=ƒÜÇ About to call setIdeaBank with:', ideas.length, 'ideas')
      console.log('=ƒÜÇ Ideas array content:', ideas)
      ...
      setIdeaBankVersion(prev => prev + 1) // Force re-render
      console.log('G£à Idea Bank setIdeaBank called with', ideasToSet.length, 'ideas, version:', ideaBankVersion + 1)
      ...
      setTimeout(() => {
        console.log('=ƒöä Re-checking ideaBank state after setState:', ideaBank.length, 'ideas, version:', ideaBankVersion)
        console.log('=ƒöä Current ideaBank content:', ideaBank)
      }, 100)
        ...
        <div className="bg-white rounded border p-4">
          ...
          <div className="text-sm text-gray-700">
            ...
            <details className="mb-2">
              ...
              <div className="mt-2 whitespace-pre-wrap text-gray-700">{abstract || 'GÇö'}</div>
            </details>
            <div className="mb-2">
              ...
              {showCustomQuery && (
                <div className="mt-3 space-y-2">
                  <textarea
                    ...
                    value={customQuery}
                    onChange={(e)=>{
                      console.log('=ƒô¥ Custom query changed from:', customQuery, 'to:', e.target.value)
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              >{searching ? '=ƒö¼ Analyzing Millions of Patents...' : busy ? 'Processing...' : 'Search Related Patents'}</button>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3">
                {!isManualPriorArtSaved ? (
                  ...
                  >
                    {savingManualPriorArt ? '=ƒÆ+ Saving...' : '=ƒÆ+ Save'}
                  </button>
                ) : (
                  <>
                    {!isEditingManualPriorArt ? (
                      ...
                      <>
                        ...
                        >
                          {savingManualPriorArt ? '=ƒÆ+ Saving...' : '=ƒÆ+ Save Changes'}
                        </button>
                        <button
                          onClick={() => {
                            setIsEditingManualPriorArt(false)
                ...
                {isManualPriorArtSaved && !isEditingManualPriorArt && (
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {savingManualPriorArt ? '=ƒÆ+ Saving...' : '=ƒÆ+ Update Selection'}
                      </button>
                      <button
                        ...
                        <div className="mt-3 border-t border-gray-200 pt-3">
                          <div className="flex items-center gap-2 mb-3">
                            <span className={`inline-block px-3 py-1.5 rounded-lg text-sm font-semibold ${
                              ...
                              =ƒº¬ Novelty Threat: {(selected[key]?.noveltyThreat || aiAnalysis[key]?.noveltyThreat || 'unknown').charAt(0).toUpperCase() + (selected[key]?.noveltyThreat || aiAnalysis[key]?.noveltyThreat || 'unknown').slice(1)}
                            </span>
                          ...
                          {(selected[key]?.aiSummary || aiAnalysis[key]?.aiSummary) && (
                            <div className="text-sm text-gray-800 bg-blue-50 p-4 rounded-lg border border-blue-200">
                              <div className="font-semibold text-blue-800 mb-2">=ƒñû AI Relevance Analysis:</div>
                          ...
                          {(() => {
                            ...
                            <div className="text-sm text-gray-800 bg-green-50 p-4 rounded-lg border border-green-200">
                              <div className="font-semibold text-green-800 mb-2">G£à Relevant Parts:</div>
                              <ul className="text-gray-700 space-y-1">
                                {relevantParts.map((part: string, idx: number) => (
                                  <li key={idx} className="flex items-start">
                                    <span className="text-green-600 mr-2">GÇó</span>
                                    <span>{part}</span>
                                  </li>
                                ))}
                              </ul>
                          ...
                          {(() => {
                            ...
                            <div className="text-sm text-gray-800 bg-gray-50 p-4 rounded-lg border border-gray-200">
                              <div className="font-semibold text-gray-800 mb-2">G¥î Irrelevant Parts:</div>
                              <ul className="text-gray-700 space-y-1">
                                {irrelevantParts.map((part: string, idx: number) => (
                                  <li key={idx} className="flex items-start">
                                    <span className="text-gray-600 mr-2">GÇó</span>
                                    <span>{part}</span>
                                  </li>
                                ))}
                              </ul>
                          ...
                          {(selected[key]?.noveltyComparison || aiAnalysis[key]?.noveltyComparison) && (
                            <div className="text-sm text-gray-800 bg-purple-50 p-4 rounded-lg border border-purple-200">
                              <div className="font-semibold text-purple-800 mb-2">=ƒÆí Novelty Comparison:</div>
        ...
        {ideaBankOpen && (
          <div className="mt-4 bg-white rounded border p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-medium text-gray-900">=ƒÆí Generated Ideas</h3>
            ...
            {ideaBank.length > 0 ? (
              ...
              <div className="text-center py-8 text-gray-500">
                <div className="text-4xl mb-3">=ƒÆí</div>
