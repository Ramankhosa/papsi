    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex flex-col gap-6">
          <div>
            <div className="text-sm text-gray-500">Paper Session</div>
            <h1 className="text-2xl font-semibold text-gray-900">{session?.researchTopic?.title || 'Untitled Paper'}</h1>
          </div>

          <div className="grid gap-6 lg:grid-cols-[220px,1fr]">
            <Card className="p-4 h-fit">
              <div className="text-sm font-semibold text-gray-900 mb-3">Stages</div>
              <div className="grid gap-2">
                {STAGES.map(stage => (
                  <Button
                    key={stage.key}
                    variant={stage.key === currentStage ? 'default' : 'secondary'}
                    className={`justify-start ${!isStageEnabled(stage.key) ? 'opacity-70' : ''}`}
                    aria-disabled={!isStageEnabled(stage.key)}
                    title={!isStageEnabled(stage.key) ? getStageLockReason(stage.key) || 'Complete previous steps.' : undefined}
            ...
            <div className="space-y-4">
              {loading && <div className="text-sm text-gray-500">Loading session...</div>}
              {error && <div className="text-sm text-red-600">{error}</div>}
              {!loading && !error && pendingStage && stageWarning && (
                <Card className="p-4">
                  <div className="text-sm font-semibold text-gray-900">Stage locked</div>
                  <div className="text-sm text-gray-600 mt-1">{stageWarning}</div>
