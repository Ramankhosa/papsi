...
export async function POST(request: NextRequest, context: { params: { paperId: string } }) {
  try {
    ...
    const result = await deepAnalysisService.stopActive(sessionId);
    return NextResponse.json(result);
  } catch (error: any) {
    console.error('[DeepAnalysis] stop error:', error);
    return NextResponse.json({ error: error?.message || 'Failed to stop deep-analysis jobs' }, { status: 500 });
  }
}
