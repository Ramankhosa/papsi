...
export async function GET(request: NextRequest, context: { params: { paperId: string } }) {
  try {
    ...
    const result = await deepAnalysisService.getCandidates(sessionId);
    return NextResponse.json(result);
  } catch (error: any) {
    console.error('[DeepAnalysis] candidates error:', error);
    return NextResponse.json({ error: error?.message || 'Failed to fetch deep-analysis candidates' }, { status: 500 });
  }
}
