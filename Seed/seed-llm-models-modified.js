...
async function main() {
  ...
  try {
    for (const plan of plans) {
      ...
      console.log(`  📝 Configuring ${plan.code} with production token limits...`);
      ...
      console.log(`  ✅ ${plan.code} configured (${configuredCount} stages with production token limits)`);
    }
  } catch (error) {
    if (error.code === 'P2021' || error.message.includes('does not exist')) {
