 ...
 * 
 * This script migrates country-specific top-up prompts from JSON files
 * to the database. These prompts are merged with superset base prompts
 * during drafting to create jurisdiction-specific outputs.
 * 
 * Run with: node Seed/seed-section-prompts.js
 * 
 * Options:
...
async function verifySectionPrompts(countryCode) {
  console.log(`\n=== Verifying ${countryCode || 'All'} Section Prompts ===\n`)

  const where = countryCode ? { countryCode: countryCode } : {}
  const prompts = await prisma.countrySectionPrompt.findMany({
    where,
    orderBy: [{ countryCode: 'asc' }, { sectionKey: 'asc' }]
  })

  console.log('Country'.padEnd(8) + 'Section Key'.padEnd(28) + 'Version'.padEnd(10) + 'Status'.padEnd(10) + 'Preview')
  console.log('-'.repeat(110))

  for (const prompt of prompts) {
    const preview = prompt.instruction.substring(0, 35).replace(/\n/g, ' ') + '...'
    console.log(
      prompt.countryCode.padEnd(8) +
      prompt.sectionKey.padEnd(28) +
